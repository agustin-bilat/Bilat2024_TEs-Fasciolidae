...
function merge_ovlp_TEs_by_fam() { 
	mkdir dir_temporal
	cd dir_temporal
	grep -v "Simple_repeat" ./../$1 | awk '$10 !~/rich/{print $0}' | awk '$11!~/RNA/{print}' |  awk '$10 !~/polypurine/ && NR>3 {print $5,$6-1,$7,$10,".",$9}' | \
 awk -v OFS="\t" '$6 ~ /C/{print $1,$2,$3,$4,".","-"} $6 ~/\+/{print $1,$2,$3,$4,".",$6}' | \
 sort -k1,1 -k2,2n > $1.bed  #convert rpmk.out to bed format and remove repeats others than TEs
...
# Calculate total TE family coverage
function calculate_TE_family_size() {
grep -v "(.*)" all.merge.sort | grep -v "rich" | awk '($3-$2) >= 1 {print $4,$3-$2}' | sort -k 2,2n > $rpmk_out.bed.tmp
awk -v g_size="$genome_size" 'NR==FNR{Arr[$1]=Arr[$1]+$2} NR!=FNR {print $1,Arr[$1],(Arr[$1]/g_size)*100}' \
$rpmk_out.bed.tmp TEnames.tmp >> TE_family_size   #format: TE_name |  TE_coverage(bp) | TE_coverage (%)
...
awk -v OFS="\t" -v feature="genes" 'BEGIN{print "TE_name","TE_coverage_bp","TE_coverage_%","Overlap_with_"feature"_bp","Overlap_with_"feature"_%","Complement_Overlap_bp"} \
NR==FNR{Arr[$1]=$2} NR!=FNR {print $1,$2,$3,Arr[$1],(Arr[$1]*100)/$2,$2-Arr[$1]}' TE_genes_intersect TE_family_size > TE_genes_intersect.tab
