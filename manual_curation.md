...
3. `for i in *fam*.fa; do bash mkfasta_from_megablast.sh <genome_assembly_name>.fna $i 0 1000 1000 <chromsizes>.fa <genome_assembly_database_prefix> ; done`
...
The alignments are manually edited with _aliview_ (see the paper's methods section). For families were the borders of the alignment are not reached, we repeat the steps 3 to 6 changing the values of the command in step 3 in order to increase the flanking sequence of the blast hits to be extracted as fasta (in the example 1 kb at either side).  
